      ...
      <div id="app" class="row pr-3">
        <div id="preview" class="col pr-0">
          <div class="card bg-transparent mr-5 position-absolute">
            ...
            <div id="mask" class="position-absolute" :class="show?'':'d-none'" :style="maskStyle"></div>
            <div id="super-mask" class="position-absolute" @mouseenter="changeShow" @mouseleave="changeShow"
            @mousemove="mousemove"></div>
            <div id="div-lg" class="position-absolute" :style="{'background-image':`url(${pics[i].lg})`,'background-position':`${-16/7*parseInt(maskStyle.left)}px ${-16/7*parseInt(maskStyle.top)}px`}"  :class="show?'':'d-none'"></div>
            <div class="card-body p-0 text-center">
              <img src="img/product_detail/hover-prev.png" class="btn float-left btn-light border-0 p-1 pt-4 pb-4" :class="moved==0?'disabled':''" @click="move(-1)" id="btnLeft">
              <div class="d-inline-block pt-2 mx-0 m-auto">
                <!-- <ul class="list-unstyled mb-0"> -->
                <ul class="list-unstyled mb-0" id="ulImgs" :style="{width:pics.length*62+'px', 'margin-left':-62*moved+'px'}">
                  <li v-for="(p,i) of pics" :key="i" class="float-left p-1">
                    <img @mouseenter="changei(i)" :src="p.sm">
                  </li>
                </ul>
              </div>
              <img src="img/product_detail/hover-next.png" class="btn float-right btn-light border-0 p-1 pt-4 pb-4"  :class="moved==pics.length-4||pics.length<=4?'disabled':''" @click="move(+1)" id="btnRight">
            </div>
          </div>
        </div>
        <div id="details" class="col pl-0">
          <h6 id="ptitle" class="font-weight-bold">
            {{product.title}}
          </h6>
          <h6>
            <a class="small text-dark font-weight-bold" href="javascript:;"  id="p_sub_title">{{product.subtitle}}</a>
          </h6>
          ...
          <div>
            ...
            <div class="float-left w-75" id="specs">
              ...
              :class="sp.lid==lid?'active':''" :to="'/details/'+sp.lid">{{sp.spec}}</router-link>
            </div>
...
<script>
import funs from "@/assets/funs.js"
...
export default {
  ...
  methods:{
    ...
    move(i){
      if(!(i==-1&&this.moved==0)&&!(i==1&&this.moved==this.pics.length-4)&&(this.pics.length>4)){
        this.moved+=i;
      }
    },
    load(){
      funs.getDetails(this.lid,result=>{
  ...
  created(){ //只在首次创建组件时执行一次，之后每次更改地址栏中的变量,created不再重复执行
    this.load();
  },
  watch:{
    lid(){
